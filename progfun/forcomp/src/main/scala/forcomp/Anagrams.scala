...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = if (s.nonEmpty) wordOccurrences(s.reduce(_.concat(_))) else List()
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    val solutions = ListBuffer[Occurrences](List())

    def backtrack(occurrences: Occurrences, partial: Occurrences) {
      for {
        item <- occurrences
        i <- 1 to item._2
      } {
        solutions.append(partial :+ (item._1, i))
        backtrack(occurrences.filter(_._1 > item._1), partial :+ (item._1, i))
      }
    }

    backtrack(occurrences, List())
    solutions.toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap) { (map, pair) =>
      val (key, freq) = pair
      if (map(key) == freq) map - key
      else map updated(key, map(key) - freq)
    }.toList.sortBy(_._1)
  }
